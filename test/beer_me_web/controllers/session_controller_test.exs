defmodule BeerMeWeb.SessionControllerTest do
  use BeerMeWeb.ConnCase

  import BeerMeWeb.AuthCase
  alias BeerMe.Accounts

  @create_attrs %{first_name: "Robin", last_name: "Example", email: "robin@example.com", password: "reallyHard2gue$$"}
  @invalid_attrs %{first_name: "Robin", last_name: "Example", email: "robin@example.com", password: "cannotGue$$it"}

  setup %{conn: conn} do
    conn = conn |> bypass_through(BeerMeWeb.Router, [:browser]) |> get("/")
    user = add_user("Robin", "Example", "robin@example.com")
