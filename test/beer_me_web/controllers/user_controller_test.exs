defmodule BeerMeWeb.UserControllerTest do
  use BeerMeWeb.ConnCase

  import BeerMeWeb.AuthCase
  alias BeerMe.Accounts

  @create_attrs %{first_name: "Bill", last_name: "Exampleson", email: "bill@example.com", password: "hard2guess"}
  @update_attrs %{first_name: "Will", last_name: "Exampleson", email: "william@example.com"}
  @invalid_attrs %{first_name: "Bill", last_name: "Exampleson", email: nil}

  setup %{conn: conn} = config do
    conn = conn |> bypass_through(BeerMeWeb.Router, [:browser]) |> get("/")
    if email = config[:login] do
      user = add_user("Johnny", "McJohnnyson", email)
      other = add_user("Tony", "McTonyson", "tony@example.com")
